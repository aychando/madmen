...
// this is middleware that allows you to get the request before the controller function
// setting this up to strip off properties
app.use(function(req, res, next){
  // res.locals takes in obj where properties are exposed to the view
  // any function defined res.locals is accessible in the views
  res.locals({
    stripProperties : function(doc, params){
      ...
      return _.omit(doc.toObject(), params);
    }
  });
  // next continues our chain to the next function, which in this case is the controller
  next();
})
...
var controller = {
  ...
  edit : function(option, req, res) {
    ...
    model.findById(modelId, function(err, doc){
      res.render('edit', {
        // doc is the data object being returned from db
        model : doc,
        action : option.basePath + "/" + modelId,
        fullPath : option.basePath
      });
